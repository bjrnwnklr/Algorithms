...
def gen_grid():
    grid = {
        (c, r): 1 if c == 0 or r == 0 or c == GRID_SIZE - 1 or r == GRID_SIZE - 1 else 0
...
def run_example():
    ...
    if came_from[target]:
        ...
        print(
            "From %s to %s: %d steps" % (str(start), str(target), cost_so_far[target])
        )
    else:
...
def a_star(grid, start, target):
    ...
    while frontier:
        ...
        for v_next in neighbours(grid, current):
            ...
            )  # cost for moving in grid is always 1 - change if there is a specific cost for movements e.g. for terrain
